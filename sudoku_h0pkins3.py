    ...
    def forward_check(self, row, column, value, mode='remove'):
        ...
        if mode == 'remove':
            ...
            for i in range(0, len(self.cells)):
                if i != row and not self.cells[i][column].remove_value(value):
                    return False
                if i != column and not self.cells[row][i].remove_value(value):
                    return False
            for i in range(0, 9):
                cell_row, cell_column = self.get_row_column(grid, i)
                if cell_row != row and cell_column != column and not self.cells[cell_row][cell_column].remove_value(value):
                    return False
            return True
        elif mode == 'count':
            count = 0
            ...
            for i in range(0, len(self.cells)):
                if i != row and value in self.cells[i][cell].domain:
                    count += 1
                if i != column and value in self.cells[row][i].domain:
                    count += 1
                if i != cell:
                    cell_row, cell_column = self.get_row_column(grid, i)
                    if value in self.cells[cell_row][cell_column].domain and cell_row != row and cell_column != column:
                        count += 1
            return count
...
def mrv(puzzle, unassigned):
    ...
    length = 9
    for x in unassigned:
        numberOfValues = len(puzzle.cells[x[0]][x[1]].domain)
        if numberOfValues < length:
            length = numberOfValues
            minimum_remaining_value = []
            minimum_remaining_value.append(x)
        elif numberOfValues == length:
            minimum_remaining_value.append(x)
...
def count_constraints(puzzle, row, column):
    ...
    count = 0
    for i in range(0, len(puzzle.cells)):
        if i != row and puzzle.cells[i][column].value is None:
            count += 1
        if i != column and puzzle.cells[row][i].value is None:
            count += 1
        if i != cell:
            cell_row, cell_column = puzzle.get_row_column(grid, i)
            if puzzle.cells[cell_row][cell_column].value is None and cell_row != row and cell_column != column:
                count += 1
...
def order_values(puzzle, row, column):
    ...
    for value in domain:
        count = puzzle.forward_check(row, column, value, 'count')
        sortedList.append((value, count))

    sortedList.sort(key = lambda x: x[1])
    domain = [num[0] for num in sortedList]
...
def backtracking_search(puzzle):
    ...
    values = order_values(puzzle, row, column)
    ...
    for value in values:
        ...
        new_puzzle.cells[row][column].assign_value(value)
        ...
        result = new_puzzle.forward_check(row, column, value)
        ...
        backtracking_result = backtracking_search(new_puzzle)
